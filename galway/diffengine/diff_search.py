...
def diff_search(start, end, diffPairs):
    if start == end:
        return [end]
    for hueristic, transformation in diffPairs:
        try:
            if hueristic(start):
                return [start] + diff_search(transformation(start), end, diffPairs)
        except NoPathException as e:
            # Continue to the next hueristic, or fail if none work
            pass
    raise NoPathException('The theorem {} cannot be proven from {}'.format(start, end))
